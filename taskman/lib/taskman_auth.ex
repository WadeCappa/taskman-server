defmodule Taskman.Auth do
  ...
  def call(conn, _opts) do
    ...
    if header == [] do
      send_resp(conn, 401, "{\"message\": \"Auth header not provided, user format 'Authorization: Bearer <token>'\"}")
    else
      {:ok, resp} = HTTPoison.put("authman:4002/check", "", [header])
      if Integer.floor_div(Map.get(resp, :status_code, 500), 100) == 2 and Map.has_key?(resp, :body) do
